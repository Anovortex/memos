...
func TestInviteToken(t *testing.T) {
	...
	t.Run("round trip", func(t *testing.T) {
		token, expiresAt, err := GenerateInviteToken("invited@example.com", secret)
	...
	t.Run("requires an email", func(t *testing.T) {
		_, _, err := GenerateInviteToken("", secret)
		assert.Error(t, err)
	})
	...
	t.Run("fails with a tampered signature", func(t *testing.T) {
		token, _, err := GenerateInviteToken("invited@example.com", secret)
		require.NoError(t, err)
	...
	t.Run("fails with a tampered payload", func(t *testing.T) {
		token, _, err := GenerateInviteToken("invited@example.com", secret)
		require.NoError(t, err)
		other, _, err := GenerateInviteToken("other@example.com", secret)
		require.NoError(t, err)
		parts := strings.Split(token, ".")
