...
func newAuthToken() (string, string, error) {
	token, err := random.String(authTokenLength)
	if err != nil {
		return "", "", err
	}
...
func (s *APIV1Service) SendEmailVerification(ctx context.Context, _ *v1pb.SendEmailVerificationRequest) (*emptypb.Empty, error) {
	...
	if err := s.throttleAndCharge(ratelimit.ScopeEmailVerifyUser, userKey(user.ID), 1); err != nil {
		return nil, err
	}
	verified, err := s.Store.IsEmailVerified(ctx, user)
	if err != nil {
...
func (s *APIV1Service) VerifyEmail(ctx context.Context, request *v1pb.VerifyEmailRequest) (*emptypb.Empty, error) {
	if err := s.throttleAndCharge(ratelimit.ScopeTokenIP, clientip.FromContext(ctx), 1); err != nil {
		return nil, err
	}
...
func (s *APIV1Service) RequestPasswordReset(ctx context.Context, request *v1pb.RequestPasswordResetRequest) (*emptypb.Empty, error) {
	...
	if err := s.throttleAndCharge(ratelimit.ScopePasswordResetIP, clientip.FromContext(ctx), 1); err != nil {
		return nil, err
	}
	if err := s.throttleAndCharge(ratelimit.ScopePasswordResetEmail, address, 1); err != nil {
		return nil, err
	}
