...
func (d *DB) IncrementUserAIUsage(ctx context.Context, userID int32, usageDate string, tokens int64) error {
	stmt := "INSERT INTO user_ai_usage (user_id, usage_date, tokens) VALUES ($1, $2, $3) " +
		"ON CONFLICT (user_id, usage_date) DO UPDATE SET tokens = user_ai_usage.tokens + EXCLUDED.tokens"
...
func (d *DB) GetUserAIUsage(ctx context.Context, userID int32, usageDate string) (int64, error) {
	...
	err := d.db.QueryRowContext(ctx,
		"SELECT tokens FROM user_ai_usage WHERE user_id = $1 AND usage_date = $2",
		userID, usageDate,
	).Scan(&tokens)
