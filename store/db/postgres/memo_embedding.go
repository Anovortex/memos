...
func (d *DB) UpsertMemoEmbedding(ctx context.Context, upsert *store.MemoEmbedding) (*store.MemoEmbedding, error) {
	stmt := "INSERT INTO memo_embedding (memo_id, creator_id, model, vector, updated_ts) VALUES (" + placeholders(5) + ") " +
		"ON CONFLICT (memo_id) DO UPDATE SET creator_id = EXCLUDED.creator_id, model = EXCLUDED.model, vector = EXCLUDED.vector, updated_ts = EXCLUDED.updated_ts"
...
func (d *DB) DeleteMemoEmbedding(ctx context.Context, delete *store.DeleteMemoEmbedding) error {
	_, err := d.db.ExecContext(ctx, "DELETE FROM memo_embedding WHERE memo_id = $1", delete.MemoID)
	return err
}
...
func (d *DB) ListMemosNeedingEmbedding(ctx context.Context, model string, limit int) ([]*store.Memo, error) {
	rows, err := d.db.QueryContext(ctx, `
		SELECT
			...
			m.content
		FROM memo m
		LEFT JOIN memo_embedding e ON e.memo_id = m.id AND e.model = $1
		WHERE e.memo_id IS NULL AND m.row_status = 'NORMAL'
		ORDER BY m.id ASC
		LIMIT $2`,
		model, limit,
	)
