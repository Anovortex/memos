...
import SetPackageDialog from "@/components/SetPackageDialog";
...
const packageSetting = (plan: UserSetting_PackageSetting_Plan, expire?: Date) =>
  create(UserSettingSchema, {
    name: packageName,
    value: { case: "packageSetting", value: { plan, expireTime: expire ? timestampFromDate(expire) : undefined } },
  });

describe("<SetPackageDialog>", () => {
  beforeEach(() => {
  ...
  it("shows the member's current package", async () => {
    ...
    render(<SetPackageDialog user={alice} open onOpenChange={vi.fn()} />);

    expect(mocks.getUserSetting).toHaveBeenCalledWith({ name: packageName });
    await waitFor(() => expect(screen.getByLabelText("setting.member.package-expires")).toHaveValue("2027-01-01"));
    expect(screen.getByRole("radio", { name: "setting.member.package-teams" })).toHaveAttribute("aria-checked", "true");
  });
  ...
  it("saves Teams with the chosen day as the end of that day", async () => {
    ...
    render(<SetPackageDialog user={alice} open onOpenChange={onOpenChange} />);
    await waitFor(() => expect(mocks.getUserSetting).toHaveBeenCalled());
    fireEvent.click(screen.getByRole("radio", { name: "setting.member.package-teams" }));
    fireEvent.change(screen.getByLabelText("setting.member.package-expires"), { target: { value: "2027-06-30" } });
    ...
    expect(mocks.toastSuccess).toHaveBeenCalledWith("setting.member.package-updated");
